...
STATUS_URL = "https://kesif646-cpu.github.io/ntub-badminton-monitor/"
...
def extract_count(row):
    ...
    # 北商目前的課程結果固定為 10 欄：
    # [8] 是「上限/已選人數」，例如 36/36。
    # 已選人數是斜線後面的數字。
    if n == 10:
        m = re.fullmatch(r"\s*(\d{1,3})\s*/\s*(\d{1,3})\s*", texts[8])
        if m:
            capacity = int(m.group(1))
            selected = int(m.group(2))
            if 0 <= selected <= max(capacity, selected):
                return selected, "欄位「上限/已選人數」"
...
def send_ntfy(title: str, message: str, priority: int = 4) -> None:
    """透過 ntfy.sh 發送手機推播。Topic 從 GitHub Secret NTFY_TOPIC 讀取。"""
    topic = (os.getenv("NTFY_TOPIC") or "").strip()
    if not topic:
        raise RuntimeError("尚未設定 GitHub Secret：NTFY_TOPIC")
    ...
    with urllib.request.urlopen(req, timeout=20) as resp:
        if not (200 <= resp.status < 300):
            raise RuntimeError(f"ntfy 回傳 HTTP {resp.status}")


def send_change_notification(previous: int, current: int, checked_at: str) -> None:
    if current < previous:
        title = "🏸 北商羽球有名額變化"
        priority = 5
    else:
    ...
    message = (
        f"{previous} 人 → {current} 人\n"
        f"90240991 羽球(上)｜黃晉揚\n"
        f"{checked_at}"
    )
    send_ntfy(title, message, priority=priority)
...
def render_page(result: dict, state: dict):
    ...
    # 手機頁面不顯示詳細錯誤框。
    # 詳細錯誤仍會保留在 GitHub Actions 的 monitor_output.txt / debug artifacts。
    error_box = ""
...
def main():
    ...
    if args.test_notification:
        try:
            ...
            print("NTFY_TEST=SUCCESS")
            return 0
        except Exception as exc:
            print(f"NTFY_TEST=FAILED: {type(exc).__name__}: {exc}", file=sys.stderr)
            return 1
    ...
    try:
        ...
        if previous is None:
            ...
            state_changed = True
            print(f"BASELINE_CREATED={current}")

        elif int(previous) != current:
            previous = int(previous)

            # 先確認推播成功，再更新基準。
            # 若 ntfy 暫時失敗，保留舊基準，下次 5 分鐘後會再嘗試通知，
            # 避免因一次網路錯誤永久漏掉變化。
            try:
                send_change_notification(previous, current, result["checked_at"])
                print(f"NTFY_CHANGE_SENT={previous}->{current}")
                ...
                print(
                    f"NTFY_CHANGE_FAILED={type(notify_exc).__name__}: {notify_exc}",
                    file=sys.stderr,
                )
                # 不更新 last_count，讓下一次排程重試推播
