...
COUNT_HEADER_EXCLUDE = ["上限", "限修", "限額", "容量", "名額", "最低", "最高"]
...
def target_row(page):
    ...
    for i in range(rows.count()):
        ...
        try:
            ...
            continue
        if COURSE_ID in text and TEACHER in text and COURSE_NAME in text:
            score = 0
            if COURSE_NAME_FULL in text:
                score += 10
            if CLASS_CODE in text:
                score += 5
            if "體育興趣選項" in text:
                score += 3
            matches.append((score, row, text))

    if not matches:
        raise RuntimeError(
            f"已執行查詢，但結果中找不到：{COURSE_ID} / {COURSE_NAME_FULL} / {TEACHER}"
        )
...
def scrape():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 先試課號，再試「羽球」
            last_error = None
            for query in [COURSE_ID, COURSE_NAME]:
                try:
                    ...
                    if COURSE_ID not in page.content():
                        click_search(page)
                        page.wait_for_timeout(2200)

                    if COURSE_ID in page.content():
                        break
            ...
            if COURSE_ID not in page.content() and last_error:
                raise last_error
